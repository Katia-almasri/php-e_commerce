<?php  
 ...
 $start_from = ($page - 1)*$record_per_page; 
  ...
  $stmt = $pdo->prepare("SELECT * FROM procomp UNION ALL SELECT * FROM 
    proclient ORDER BY date_of_expose DESC LIMIT {$start_from},{$record_per_page}");
  $stmt->execute();
...
<hr>
  ...
  $stmt1 = $pdo->prepare("SELECT * FROM procomp UNION ALL SELECT * FROM 
    proclient ORDER BY date_of_expose DESC ");
  $stmt1->execute();
 ...
 {  
      $output .= "<span class='pagination_link_pl_1' style='cursor:pointer; padding:6px; border:1px solid #ccc;' id='".$i."'>".$i."</span>";  
 }  
 $output .= '</div><br /><br />';  
 echo $output;  
 ?>  