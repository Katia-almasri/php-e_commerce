...
$type = "";
                                 /*should be visitor_type(client or company) with id, name, his profile image
...
try {
    ...
    // if the product was not in procomp table so it is in proclient table  
    if(!$product){                        
    $sql    = "SELECT * FROM proclient WHERE pro_id = $pro_id";
    $stmt   = $pdo->prepare($sql);
    $stmt->execute();
    $product = $stmt->fetch();
    $type = "client";
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $("#leave_comment").click(function(event){
                ...
                comment_field = $("#new-review").val();
                $.ajax({
                    type: "POST",
                    url: "processComment.php",
                    data:{
                        //here the name of applicant(company or client (by knowing its id retrieve name))
                        name:"<?php echo($name);?>", 
                        comment_field :comment_field,
                        type_product = "<?php echo($type);?>",
                        type_id = "<?php echo($pro_id);?>",
                        //here the id of applicant(company or client)
                        com_id = "<?php echo($com_id);?>"
                    },
                    success: function(data){
                        new_comment = '<div class="d-flex justify-content-between align-items-center"><div class="user d-flex flex-row align-items-center"><img src="<?php echo($image);?>" width="30" class="user-img rounded-circle mr-2"> <span><small class="font-weight-bold text-primary">'+name+'</small> <small class="font-weight-bold">'+comment_field+'</small></span> </div> <small>just now';
                    $(".card card1 p-3").repend(new_comment);
        ...
        <div class="container dark-grey-text mt-5">
            ...
            <div class="row wow fadeIn">
                <div class="col-md-6 mb-4">
                    <img src="../DBHolder/<?= $product['image'] ?>" class="img-fluid" alt="">
                </div>
                <div class="col-md-6 mb-4">
                    ...
                    <div class="p-4">
                        ...
                        <form action="CartAndCheckout.php" method="post">
                            <input type="number" name="quantity" value="1" min="1" max="<?=$product['amount']?>" placeholder="Quantity" required>   
    ...
    <div class="container mt-5">
        <div class="row d-flex justify-content-center">
            <div class="col-md-8">
                ...
                <?php
                        $comments = new DBManager();
                        if($type==='company'){
                            $result = $comments->selectQuery("SELECT procomp_comment.*, 
                            CASE 
                            ...
                            FROM procomp_comment WHERE procomp_id = '$pro_id' LIMIT 10;");
                ...
                <div class="card card1 p-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="user d-flex flex-row align-items-center"> <img src="<?php echo($Comments[$i]['comment_image']);?>" width="30" class="user-img rounded-circle mr-2"> <span><small class="font-weight-bold text-primary"><?php echo $comments[$i]['comment_name'];?></small> <small class="font-weight-bold"><?php echo $comments[$i]['comment_text'];?></small></span> </div> <small><?php  $day = $comments[$i]['comment_name'];
                        $today =  date("Y-m-d");
                        $TodayFormat = new DateTime($today);
                        $date = new DateTime($day);
                        if((int)$today->format("D") - (int)$day->format("D")<30){
                            echo (int)$today->format("D") - (int)$day->format("D").'day(s) ago';
                        }else if((int)$today->format("M") - (int)$day->format("M")!=0){
                            echo (int)$today->format("M") - (int)$day->format("M").'month(s) ago';
                        }else if((int)$today->format("Y") - (int)$day->format("Y")){
                            echo (int)$today->format("Y") - (int)$day->format("Y").'year(s) ago';
                        }
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                         ...
                         <?php  }
                                ...
                                $result = $Comments->selectQuery("SELECT proclient_comment.*, 
                        CASE 
                   ...
                   <!-- <div class="action d-flex justify-content-between mt-2 align-items-center">
                        <div class="reply px-4"> <small>Remove</small>  <span class="dots"></span> <small>Translate</small> </div>
                ...
                <div class="col-md-12">
                    <form accept-charset="UTF-8" action="" method="post">
                        ...
                        <div class="text-right">
                            ...
                            <button class="btn btn-primary btn-lg" type="submit" id="leave_comment">comment</button>
                        </div>
                    </form>
                </div>
            </div>
