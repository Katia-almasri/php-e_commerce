...
require_once 'auxFunctions.php';
	...
	if(isset($_SESSION['client_id'])){
		...
		function checkRedundantProduct($client_id, $pro_name){
		$product = new ProductManager();
		$output = '';
		$is_redundant_product = $product->selectQuery("SELECT pro_name, client_id, proclient_id FROM proclient WHERE pro_name = '$pro_name' AND client_id = '$client_id'");
			...
			$output.='<input type="submit" id="'.$is_redundant_product[0]['proclient_id'].'" value="replace" class="replace">';
			$output.='<input type="submit" id="'.$is_redundant_product[0]['proclient_id'].'" value="cancel" class="cancel"></div>';
		...
		if(empty($pro_name) || empty($amount) || empty($cost)||empty($weight)){
			...
			if(!empty($production_date)){
				...
				$com_feature = $product->selectQuery("SELECT client_id, username FROM client WHERE email ='$email'");
				if($pro_feature!==NULL){               //new for admin
						$pro_id = (int)$pro_feature[0]['pro_id'];
						$com_id = (int)$com_feature[0]['client_id'];
						$client_username = $com_feature[0]['username'];
						//add notification
						$admin = new AdminManager();
						//if this company added same product name in the past(replace or cancel):
						$redundant_pro = checkRedundantProduct($client_id, $pro_name);
						if($redundant_pro!=='')
							$error_found = $redundant_pro;
						else
						{
							$date_of_not = date('Y-m-d');
							$data = $admin->addNotification('add request', 'client', $date_of_not, $client_id, $client_username, $selectedImageURL, 1, $item, $amount, $cost, $production_date, $rate, $pro_name, $col, $size, $aboutproduct, $pro_id, $weight);
					...
					$date_of_not = date('Y-m-d');
					$client_username = $com_feature[0]['username'];
					$data = $admin->addNotification('add request', 'client', $date_of_not, $client_id, $client_username, $selectedImageURL, 0, $item, $amount, $cost, $production_date, $rate, $pro_name, $col, $size, $aboutproduct, $pro_id, $weight);
